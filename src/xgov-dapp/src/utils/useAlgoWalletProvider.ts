...
class ManualSigningProvider implements CustomProvider {
  ...
  async reconnect(metadata: Metadata) {
    return null
  }
  ...
  async signTransactions(
    ...
    for (const dtxn of decodedTxns) {
      ...
      await new Promise<void>((resolve) =>
        setTimeout(() => {
          navigator.clipboard.writeText(forSigning).catch(() => alert('Copy failed; try copying from developer console'))
          resolve()
        }, 1),
      )
    ...
    },
    {
      ...
      clientOptions: {
        ...
        getProvider: (params: { network?: Network; algod?: algosdk.Algodv2; algosdkStatic?: typeof _algosdk }) => {
          return new ManualSigningProvider(params.algosdkStatic ?? algosdk)
        },
      },
    },
  ] as ProvidersArray
